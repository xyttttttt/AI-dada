...
-- 创建库
create database  if not exists dada_db;
...
(
    id                bigint auto_increment comment 'id' primary key,
    appName           varchar(512)                       null comment '应用名',
    ...
    reviewMessage     varchar(1024)                      null comment '审核信息',
...
(
    ...
    resultProp       varchar(128)                       null comment '结果属性集合JSON，如[I,S,T,J]',
    resultScoreRange int                                null comment '结果得分范围,如80分,表示80及以上命中这个结果',
    appId            bigint                             not null comment '应用 id',
    userId           bigint                             not null comment '创建用户 id',
    createTime       datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    updateTime       datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    isDelete         tinyint  default 0                 not null comment '是否删除',
    index idx_postId (appId)
) comment '得分结果' collate = utf8mb4_unicode_ci;
...
(
    ...
    appType          tinyint default 0                  not null comment '应用类型(0-得分类，1-测评类)',
    scoringStrategy  tinyint default 0                  not null comment '评分策略(0-自定义，1-AI)',
    choices          text                               null comment '用户答案(JSON数组)，如[A,B,C,D]',
